...
> Note: the bind mount path is `./data/ngnix/logs` (typo for "nginx" carried over
...
3. **Run `make`** (or `make up`). Unlike a plain `docker compose up`, this Makefile
   ...
   This runs, in order:
   - `create-volumes` — creates `${HOME}/data/mariadb` and `src/data/wordpress` on
...
| `make create-volumes` | (internal, called by `up`) creates `${HOME}/data/mariadb` and `src/data/wordpress` on the host. |
...
**Full teardown including volumes** (drops the database — bind-mounted `./data/...`
...
| `/var/www/html` (wordpress) | bind mount | `src/data/wordpress` | WordPress core, themes, plugins, uploads |
| `/var/www/html` (nginx) | bind mount | `src/data/wordpress` (same as above) | Nginx reads/serves the same files WordPress writes |
| `/var/log/nginx` (nginx) | bind mount | `src/data/ngnix/logs` | Access/error logs |

Key implications:

- **WordPress and Nginx share the same bind-mounted directory** (`src/data/wordpress`)
...
- The `Makefile`'s `create-volumes` target pre-creates `${HOME}/data/mariadb` on the
  ...
  not `${HOME}/data/mariadb`) — the host directory the `Makefile` creates is unused
...
- **To fully reset state** (e.g. testing setup from scratch), use `make fclean` (see
  ...
  ```bash
  docker-compose -f src/docker-compose.yml down -v
  sudo rm -rf ${HOME}/data/mariadb src/data/wordpress
  make up
  ```
